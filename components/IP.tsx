...
export default function IP() {
  ...
  const [lastSearchedIp, setLastSearchedIp] = useState("");
  ...
  async function runLookup() {
    ...
    try {
      ...
      const fetchPromise = (async () => {
        const res = await fetch(`/api/vt/ip?ip=${encodeURIComponent(value)}`, {
          method: "GET",
          cache: "no-store",
        });
              ...
              <div className="flex items-center justify-between gap-4 border-b border-neutral-800 px-4 py-3">
                ...
                <div className="flex items-center gap-4">
                  <div className="font-mono text-neutral-400">
                    Current IP searched:{" "}
                    <span className="text-neutral-200">{lastSearchedIp || ip}</span>
                  </div>
      ...
      <div className="w-full space-y-6">
        <div className="w-full p-5 h-full flex justify-center items-center gap-x-5">
          <input
            ...
            placeholder="8.8.8.8"
            className="w-full bg-slate-800/90 focus:ring-1 focus:ring-[#196adc] rounded-sm hover:ring-[#196adc] hover:shadow-2xl hover:shadow-[#196adc] hover:ring-1 text-neutral-200 border border-neutral-700 px-3 py-2 text-md outline-none"
          />

          <button
            onClick={runLookup}
            disabled={loading}
            className="bg-[#196adc] transition-all flex items-center justify-center p-2 cursor-pointer hover:-translate-y-1 hover:shadow-md hover:bg-blue-400 hover:shadow-[#196adc] rounded-sm disabled:opacity-100"
          >
            {loading ? (
              ...
              <HiMagnifyingGlass className="size-7 text-black" />
            )}
          </button>
        </div>
          ...
          <div className="px-5">
            <div className="border border-neutral-700 bg-[#2b3550]/60 px-4 py-3 flex items-center gap-2">
              ...
              <div className="text-sm font-mono text-red-400">
                {stats.malicious ?? 0}/{sumStats(stats)} security vendors flagged this IP
        ...
        {stats && (
          <div className="px-5 grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="border border-neutral-700 bg-neutral-900/40 p-3">
              ...
              <div className="text-lg text-emerald-400 font-mono">{stats.harmless ?? 0}</div>
            </div>

            <div className="border border-neutral-700 bg-neutral-900/40 p-3">
              <div className="text-xs text-neutral-400 font-mono">Timeout</div>
              <div className="text-lg text-neutral-400 font-mono">{stats.timeout ?? 0}</div>
            </div>
          </div>
        )}
