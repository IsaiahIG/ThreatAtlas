...
export default function Hash() {
  ...
  const [lastSearchedHash, setLastSearchedHash] = useState("");
  ...
  async function runLookup() {
    ...
    try {
      ...
      const fetchPromise = (async () => {
        const res = await fetch(`/api/vt/hash?hash=${encodeURIComponent(value)}`, {
          method: "GET",
          cache: "no-store",
        });
        ...
        if (!res.ok) throw new Error(data?.error ?? "Hash lookup failed");
              ...
              <div className="flex items-center justify-between gap-4 border-b border-neutral-800 px-4 py-3">
                ...
                <div className="flex items-center gap-4">
                  <div className="font-mono text-neutral-400">
                    Current hash searched:{" "}
                    <span className="text-neutral-200">{lastSearchedHash || hash}</span>
                  </div>
